...
if($opcion == 1){	
	...
	$sql = "SELECT id, codigo, ubicacion, capacidad, (capacidad - alquiladas) AS disponibles, alquiladas AS retornables, latitud, longitud, activo, CASE WHEN direccion IS NULL THEN '' ELSE direccion END AS direccion, CASE WHEN zipcode IS NULL THEN '' ELSE zipcode END AS zipcode, estatus AS status FROM Bases
...
if($opcion == 8){
	...
	$sql = "SELECT  W.idcliente, W.credito AS disponibilidad, WD.credito_actual AS disponibilidad_anterior, 
					...
					AB.horas,
					AB.horas, DATE_FORMAT(ADDDATE(fecha_alquiler,  INTERVAL horas HOUR), '%m/%d/%Y %h:%i:%s %p') AS deberetornar,
					DATE_FORMAT(ADDDATE(ADDDATE(fecha_alquiler,  INTERVAL horas HOUR),  INTERVAL '1' HOUR), '%m/%d/%Y %h:%i:%s %p') AS deberetornartimeextender,
...
if($opcion == 13){
	...
	$sql = "SELECT  id, CONCAT('****-****-****-', RIGHT(TC.nro_tarjeta, 4)) AS nro_tarjeta, codigo1, codigo2, '***' AS codigo_seguridad, principal, nombre, apellido, zipcode FROM tarjetas_clientes
